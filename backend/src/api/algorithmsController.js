...
router.post('/dct/encode', upload.single('image'), async (req, res) => {
  try {
    ...
    const { stegoBuffer, metrics } = await encodeDCT(imageFile.buffer, payloadBuffer, opts);
...
router.post('/dwt/encode', upload.single('image'), async (req, res) => {
  try {
    ...
    const { stegoBuffer, metrics } = await encodeDWT(imageFile.buffer, payloadBuffer, opts);
    res.json({ imageBase64: stegoBuffer.toString('base64'), mime: 'image/png', metrics });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
...
router.post('/pvd/encode', upload.single('image'), async (req, res) => {
  try {
    ...
    const { stegoBuffer, metrics } = await encodePVD(imageFile.buffer, payloadBuffer, opts);
    res.json({ imageBase64: stegoBuffer.toString('base64'), mime: 'image/png', metrics });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
